...
import gmpy2
import json
import base64 # Added this import for base64 encoding/decoding
...
class RSA():
    ...
    def __get_probable_prime(self, e:int, a:int=None, b:int = None) -> int: #there is a mechanism that generates provable primes, but we opt for probable primes
        ...
        while True:
            ...
            if q < (((2**(self.nlen//2-1)) * int(math.sqrt(2)*1e12)) //int(1e12)):
                continue
            print('f')
            if (abs(self.p-q)<((2**(self.nlen//2-100)))):
                continue
            print('g')
    ...
    def save_state(self, filename: str):
        ...
        state = {
            ...
            'n': base64.b64encode(self.__long_to_bytes(self.n)).decode('utf-8'),
            'e': self.e,
    ...
    def load_state(self, filename: str):
        ...
        with open(filename, 'r') as f: # Changed to 'r' for reading text file
            ...
            self.e = state['e']
            self.d = self.__bytes_to_long(base64.b64decode(state['d']))
...
if __name__ == "__main__":
    ...
    try:
        ...
        print("\nRSA Initialization Complete.")
        n, e = rsa.get_public_key()
        _, d = rsa.get_private_key()
        print(f"\nPublic Key (n, e): ({n}, {e})")
        print(f"Private Key (n, d): (..., {d})")

        state_filename = "rsa_keys.json"
        print(f"\nAttempting to save RSA state to '{state_filename}'...")
        try:
            rsa.save_state(state_filename)
            print("RSA state saved.")
        except TypeError as json_error:
            print(f"ERROR: Could not save RSA state due to TypeError (JSON does not support bytes directly). {json_error}")
        except Exception as e:
            print(f"An unexpected error occurred during saving: {e}")

        print(f"\nAttempting to load RSA state from '{state_filename}'...")
        try:
            rsa_loaded = RSA()
            rsa_loaded.load_state(state_filename)
            n_loaded, e_loaded = rsa_loaded.get_public_key()
            _, d_loaded = rsa_loaded.get_private_key()
            print("RSA state loaded.")
            print(f"Loaded Public Key (n, e): ({n_loaded}, {e_loaded})")
            print(f"Loaded Private Key (n, d): (..., {d_loaded})")
            if n == n_loaded and e == e_loaded and d == d_loaded:
                print("Loaded keys match generated keys. State saving/loading successful!")
            else:
                print("ERROR: Loaded keys do NOT match generated keys.")
        except FileNotFoundError:
            print(f"File '{state_filename}' not found. Cannot load state.")
        except Exception as e:
        ...
        if len(message) > max_msg_bytes_for_n:
            print(f"WARNING: Message is too long for direct encryption with current n. Truncating to {max_msg_bytes_for_n} bytes.")
            message = message[:max_msg_bytes_for_n]
        
        print("\nEncrypting message...")
        encrypted_message = rsa_for_test.encrypt(message)
        print(f"Encrypted Message (bytes, hex): {encrypted_message.hex()}")

        print("\nDecrypting message...")
        decrypted_message = rsa_for_test.decrypt(encrypted_message)
        print(f"Decrypted Message (bytes, hex): {decrypted_message.hex()}")
        print(f"Decrypted Message (decoded): '{decrypted_message.decode('utf-8', errors='ignore')}'")

        original_padded = message.rjust(len(decrypted_message), b'\x00')

        if original_padded == decrypted_message:
            print("\nRSA Test Successful: Original message matches decrypted message!")
        else:
            print("\nRSA Test Failed: Decrypted message does NOT match original message.")
            print(f"Original (padded) value: {bytes_to_long(original_padded)}")
            print(f"Decrypted value: {bytes_to_long(decrypted_message)}")
            print(f"Original (padded) bytes: {original_padded}")
            print(f"Decrypted bytes: {decrypted_message}")

    except Exception as e:
        print(f"\nAn error occurred during RSA test: {e}")
